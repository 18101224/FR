...
from .rpe_options import make_kprpe_shared, make_kprpe_input
...
def forward_original(x ,qkv, scale,proj, proj_drop,attn_drop,num_heads, rpe_k=None, rpe_q=None, rpe_v=None,extra_ctx=None,training=True):
    batch_size, num_token, embed_dim = x.shape
    #qkv is [3,batch_size,num_heads,num_token, embed_dim//num_heads]
    qkv = qkv(x).reshape(
        batch_size, num_token, 3, num_heads, embed_dim // num_heads).permute(2, 0, 3, 1, 4)
    ...
    if rpe_q is not None:
        attn += rpe_q(k * scale).transpose(2, 3)
...
def forward_flash(x ,qkv, scale,proj, proj_drop,attn_drop,num_heads, rpe_k=None, rpe_q=None, rpe_v=None,extra_ctx=None,training=True):
    B, L, C = x.shape

    # qkv: (3, B, H, L, D)
    qkv = (
        qkv(x)
        .reshape(B, L, 3, num_heads, C // num_heads)
        .permute(2, 0, 3, 1, 4)
    )
    q, k, v = qkv[0], qkv[1], qkv[2]  # (B, H, L, D)

    # --- KP-RPE bias 만들기 (logit bias: B_ij) ---
    attn_bias = None
    if rpe_k is not None or rpe_q is not None:
        # KP-RPE 원본 따라가려면 사실상 "하나의 B_ij"로 통합하는 게 제일 깔끔함.
        # 너 코드 구조를 당장 유지한다면, 둘을 더해서 bias로 만들면 됨.
        attn_bias = 0.0

        if rpe_k is not None:
            ctx = extra_ctx["rel_keypoints"]
            attn_bias = attn_bias + rpe_k(ctx)  # (B,H,L,L) 기대

        if rpe_q is not None:
            # 이 항이 진짜 KP-RPE 원문과 1:1 대응인지 애매함(너 구현 정의에 따라 다름)
            # 일단 shape만 (B,H,L,L)로 맞춰 bias에 합산
            attn_bias = attn_bias + rpe_q(k * scale).transpose(2, 3)

        # SDPA가 bias를 더해줄 때 dtype/shape가 애매하면 fallback될 수 있어서
        # q dtype에 맞춰주는 게 안전한 편
        attn_bias = attn_bias.to(dtype=q.dtype)

    # dropout: SDPA에 직접 넘김 (eval에서는 0.0)
    dropout_p = attn_drop.p if training else 0.0

    # --- SDPA (Flash/MemEff/Math 자동 선택) ---
    out = F.scaled_dot_product_attention(
        q, k, v,
        attn_mask=attn_bias,   # float bias면 logits에 더해짐
        dropout_p=dropout_p,
        is_causal=False,
    )  # (B,H,L,D)

    x = out.transpose(1, 2).reshape(B, L, C)
    x = proj(x)
    x = proj_drop(x)
    return x
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.use_flash_attn = bool(getattr(runtime_args, "use_flash_attn", False))
        self.forward_fn = forward_flash if self.use_flash_attn else forward_original
    ...
    def forward(self, x, extra_ctx=None):
        x = self.forward_fn(
            x=x,qkv=self.qkv,scale=self.scale,proj=self.proj,proj_drop=self.proj_drop,attn_drop=self.attn_drop,
            num_heads=self.num_heads,rpe_k=self.rpe_k,rpe_q=self.rpe_q,rpe_v=self.rpe_v,extra_ctx=extra_ctx,training=self.training
        )
        return x
...
class Block(nn.Module):
    def __init__(self,
        ...
        self.attn = Attention(
            ...
            rpe_config=rpe_config, runtime_args=runtime_args)
...
class VisionTransformerWithKPRPE(nn.Module):
    """
        ...
        self.blocks = nn.ModuleList(
            [
                Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                      ...
                      num_patches=num_patches, patch_n=patch_n, rpe_config=rpe_config, runtime_args=runtime_args)
    ...
    def forward_features(self, x, keypoints=None):
        B = x.shape[0]
        ...
        extra_ctx = make_kprpe_input(keypoints, x, self.keypoint_linear, self.rpe_config, self.mask_ratio,
                                     self.depth, self.num_heads, self.num_buckets)

        for block_idx, func in enumerate(self.blocks):
            if isinstance(extra_ctx, list):
                extra_ctx_ = extra_ctx[block_idx]
            else:
                extra_ctx_ = extra_ctx
            if self.using_checkpoint and self.training:
                from torch.utils.checkpoint import checkpoint
                x = checkpoint(func, x, extra_ctx_)
            else:
                x = func(x, extra_ctx=extra_ctx_)
        x = self.norm(x.float())
        if self.training and self.mask_ratio > 0:
